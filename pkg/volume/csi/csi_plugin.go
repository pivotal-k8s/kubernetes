...
func (p *csiPlugin) RegisterPlugin(pluginName string, endpoint string, versions []string) error {
	...
	unregErrMsg := "RegisterPlugin, when using the CSIPlugin as a PluginHandler, failed to unregister the plugin due to a previous error: %v"

	driverNodeID, maxVolumePerNode, accessibleTopology, err := csi.NodeGetInfo(ctx)
	if err != nil {
		if unregErr := p.unregisterDriver(pluginName); unregErr != nil {
			klog.Error(log(unregErrMsg, unregErr))
		}
		return err
	}
