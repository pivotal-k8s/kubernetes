...
func portForward(client libdocker.Interface, podSandboxID string, port int32, stream io.ReadWriteCloser) error {
	...
	netshAddArgs := []string{
		...
		//"listenaddress=localhost", // "If an address is not specified, the default is the local computer."
		fmt.Sprintf("connectaddress=%s", match.IPAddress.String()),
		fmt.Sprintf("connectport=%d", port)}
	glog.Infof("running netsh %s", strings.Join(netshAddArgs, " "))
	...
	netshDelArgs := []string{
		...
		"v4tov4",
		fmt.Sprintf("listenport=%d", randomPort)}
	err = netshClient.DeletePortProxyRule(netshDelArgs)
	if err != nil {
		return err
	}
	return nil
